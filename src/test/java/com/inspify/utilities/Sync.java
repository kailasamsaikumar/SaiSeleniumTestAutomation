...
public class Sync extends TestngListener implements TimeOuts
{
	...
	**/
	public boolean waitForJavaScriptCondition(final String javaScript, int timeOutInSeconds)
	{
		...
		try
		{				
			new WebDriverWait(driver, Duration.ofSeconds(timeOutInSeconds)).until((ExpectedCondition<Boolean>) driverObject -> (Boolean) ((JavascriptExecutor) driverObject).executeScript(javaScript));
    ...
    * @throws Exception
    * */
	public boolean waitForJQueryProcessing(Duration timeOutInSeconds)
	{
		...
		{
			Thread.sleep(500);
			long seconds = timeOutInSeconds.getSeconds();
			new WebDriverWait(driver, Duration.ofSeconds(seconds)).until((ExpectedCondition<Boolean>) driverObject -> (Boolean) ((JavascriptExecutor) driverObject).executeScript("return jQuery.active == 0"));
	...
	public void waitForPageToLoad(final String text) throws Exception
	{
		...
		nullifyImplicitWait(); 
		(new WebDriverWait(driver, Duration.ofSeconds(20))).until((ExpectedCondition<WebElement>) d -> d.findElement(By.partialLinkText(text)));
		setImplicitWait(IMPLICITWAIT); 
	}
	...
	*/
	public boolean waitForPageToLoad(int timeOutInSeconds)
	{
		...
		try
		{
			new WebDriverWait(driver, Duration.ofSeconds(timeOutInSeconds)).until((ExpectedCondition<Boolean>) driverObject -> (Boolean) ((JavascriptExecutor)driver).executeScript("return document.readyState").equals("complete"));
